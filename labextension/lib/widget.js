...
const id = 'kubeflow-mlops:deploymentPanel';
/**
...
async function activate(lab, labShell, restorer, tracker, docManager) {
    ...
    if (backend) {
        try {
            await RPCUtils_1.executeRpc(kernel, 'log.setup_logging');
        }
        catch (error) {
            RPCUtils_1.globalUnhandledRejection({ reason: error });
            throw error;
        }
        try {
            await RPCUtils_1.executeRpc(kernel, 'rok.check_rok_availability');
        }
        catch (error) {
            const unexpectedErrorCodes = [
                RPCUtils_1.RPC_CALL_STATUS.EncodingError,
                RPCUtils_1.RPC_CALL_STATUS.ImportError,
                RPCUtils_1.RPC_CALL_STATUS.UnhandledError,
            ];
        ...
        rokError = {
            ...
            code: RPCUtils_1.RPC_CALL_STATUS.ImportError,
